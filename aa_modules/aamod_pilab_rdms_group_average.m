        ...
        switch aap.tasklist.currenttask.settings.outputmode
            case 'searchlight'
                ...
                grouprdms_sess = cellfun(@(d) d / nsub,sumrdms_sess,...
                    'uniformoutput',false);
                % put in MriVolume (initialised with meta data from last
                % subject) and save 
                groupdisvol = MriVolume(grouprdms_mean,meanrdms);
                sessoutpaths = cell(nsess,1);
                for sess = 1:nsess
                    groupdisvol_sess = MriVolume(grouprdms_sess{sess},...
                        sessrdms{1});
